...
Options:
  ...
  --verbose              Log major actions before and after they run
...
func ValidateArgs(args []string) Options {
	...
	options.Args = rawArgs
	options.Verbose = verbose
	fail := func(message string) Options {
		failed := usageError(message)
		failed.Args = rawArgs
		failed.Verbose = verbose
		return failed
	}
	for i := 0; i < len(args); i++ {
		...
		case "--scan":
			if options.UpdateManifest || options.Lines != nil || options.SinceLastRun || options.MutateAll ||
				...
				return fail("Cannot combine --scan with --update-manifest or mutation execution options.")
			}
			options.Scan = true
		case "--update-manifest":
			if options.Scan || options.Lines != nil || options.SinceLastRun || options.MutateAll ||
				options.TimeoutFactor != 10 || options.TestCommand != DefaultTestCommand || options.MaxWorkers != 0 {
				return fail("Cannot combine --update-manifest with --scan or mutation execution options.")
			}
		...
		case "--since-last-run":
			if options.Scan || options.UpdateManifest || options.Lines != nil || options.MutateAll {
				return fail("Cannot combine --since-last-run with --scan, --update-manifest, --lines, or --mutate-all.")
			}
			options.SinceLastRun = true
		case "--mutate-all":
			if options.Scan || options.UpdateManifest || options.Lines != nil || options.SinceLastRun {
				return fail("Cannot combine --mutate-all with --scan, --update-manifest, --lines, or --since-last-run.")
			}
	...
	if options.SourcePath == "" {
		return fail("Missing source file argument.")
	}
	if _, err := os.Stat(options.SourcePath); err != nil {
		return fail("Source file not found: " + options.SourcePath)
	}
	return options
}
